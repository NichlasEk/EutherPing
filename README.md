...
The current release is `0.8.24`; the detailed `0.8.0` overview below describes
...
This is deliberately labelled **Ratchet Beta**. EP3 text has per-message ratchet advancement through Vodozemac, but EutherPing does not claim independently reviewed forward secrecy or post-compromise security yet. A confirmed identity-change reset deletes only the selected contact's old ratchet and pinned identity, accepts the pending replacement invitation, and requires both phones to compare the new safety code. Group encryption, secure backups, ratcheted attachments, and atomic receive-state/plaintext commits remain future work. Authenticated frames have bounded replay and stale-frame rejection; unexpected identity changes are quarantined. Carrier metadata, delivery records, multipart-SMS cost, and the fact that two numbers communicate remain visible to the mobile network. Carrier MMS remains a separate unencrypted telephony feature.
