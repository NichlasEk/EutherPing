...
- Ordinary text and pending-image draft references are stored in an app-private carrier-draft store. Vessel draft text is ciphertext in the Android-Keystore-protected Secure vault, while its non-content presence flag uses a separate Secure index; neither enters the ordinary draft store. Drafts are cleared after a successful send or explicit draft removal.
