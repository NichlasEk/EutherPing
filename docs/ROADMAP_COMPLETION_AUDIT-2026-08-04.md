...
| 6 — reviewed ratcheting protocol | Migration design, candidate assessment, provider-neutral API, isolated libsignal adapter, measured comparison, always-included Vodozemac 0.10.0 provider, signed EP3I pre-key invitations, encrypted EP3A acceptance, ratcheted EP3M text, ratcheted EP3F attachment manifests with encrypted direct payload transfer, no-downgrade peer state, safety-code binding, encrypted `secure_sessions_v3`, app-start identity initialization, persistent Alice/Bob Android tests, threat/recovery cases, and external-review packet. | **Paired-device beta in 0.8.13; incomplete release gate.** Physical force-stop/reboot and Samsung/GrapheneOS carrier and Wi-Fi/Bluetooth interoperability, atomic receive-state/plaintext commit, verified reset UX, distribution review, external report, and closed remediation list remain. Ratchet Beta must remain. |
...
3. Run the 0.8.13 opt-in EP3 beta on Samsung and GrapheneOS, including fresh
