...
| 6 — reviewed ratcheting protocol | Migration design, candidate assessment, provider-neutral API, pinned isolated libsignal 0.99.4 adapter, measured comparison, always-included Vodozemac 0.10.0 `SecureProtocolProvider`, signed identity frames, real Keystore/AtomicFile `secure_sessions_v3`, app-start identity initialization, persistent Alice/Bob Android tests, threat/recovery cases, and external-review packet. | **Incomplete.** The provider/store now initialize in the app, but EP3 carrier framing, practical authenticated prekey delivery, physical force-stop/reboot and two-phone interoperability, distribution review, external report, and closed remediation list do not exist. Secure Beta must remain. |
